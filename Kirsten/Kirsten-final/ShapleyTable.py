...
# Helper function
def draw_cell(ax, x, y, width, height, text, bg_color, text_color='black', bold=False, fontsize=10):
    rect = Rectangle((x, y), width, height, facecolor=bg_color, edgecolor='black', linewidth=1)
    ax.add_patch(rect)
    ax.text(
        x + width/2, y + height/2, text,
        ha='center', va='center',
        fontsize=fontsize, weight='bold' if bold else 'normal', color=text_color
    )
...
for i, h in enumerate(headers):
    draw_cell(ax, current_x, current_y, col_widths[i], row_height, h,
              '#34495E', 'white', True)
    current_x += col_widths[i]

# ------------------------------
# FIXED ROW ORDER (MATCHES CORRELATION TABLE)
# ------------------------------
...
# ------------------------------
# YOUR SHAPLEY VALUES REORDERED INTO:
# ALCOHOL (Teen, Adult), MARIJUANA (Teen, Adult), NICOTINE (Teen, Adult)
# ------------------------------
...
ax.legend(handles=legend_elements, loc='upper left',
          bbox_to_anchor=(0, 0.05), frameon=False, ncol=4, fontsize=10)

plt.tight_layout()
plt.savefig('shapley_table_fixed.png', dpi=300, bbox_inches='tight', facecolor='white')
plt.show()

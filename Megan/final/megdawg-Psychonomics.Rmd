...
df1 <- df1 %>%
  dplyr::rename(obj_class = Minutes_class) %>%
  mutate(
...
df1 <- df1 %>%
  dplyr::rename(sas_class_desc = sas_group) %>%
  mutate(
    sas_class = case_when(
...
# -------------------------------------------------------------
# STEP 1: Split digital immersion PCA score into high/low groups
# -------------------------------------------------------------
library(dplyr)

df1 <- df1 %>%
  mutate(
    # Compute median digital immersion PCA score
    digital_median = median(screen_behavior_pca_score, na.rm = TRUE),
    
    # Create grouping variable based on median
    digital_class = case_when(
      is.na(screen_behavior_pca_score) ~ NA_integer_,
      screen_behavior_pca_score <= digital_median ~ 1,  # Low immersion
      screen_behavior_pca_score >  digital_median ~ 2   # High immersion
    ),
    
    # Create readable labels
    digital_class_desc = case_when(
      digital_class == 1 ~ "Low_Digital",
      digital_class == 2 ~ "High_Digital",
      TRUE ~ NA_character_
    )
  )
```
...
```{r}
# -------------------------------------------------------------
# STEP 2: MANCOVA with sleep moderation
# -------------------------------------------------------------
library(car)
library(broom)
library(purrr)
...
df1 <- df1 %>%
  mutate(
    digital_class = factor(digital_class, levels = c(2, 1)),  # High first
    sleep_composite.z = as.numeric(sleep_composite.z),
    Age = as.numeric(Age)
  )

# Define DVs
dv_vars <- c("barrets.z", "delay_discounting_indiff_z", "texi_sum_score.z", 
             "Flanker_Z", "dprime.z")

# Filter for complete cases
df_mancova <- df1 %>%
  dplyr::select(all_of(dv_vars), digital_class, sleep_composite.z, Age) %>%
  na.omit()

# Run MANCOVA
mancova_model <- manova(
  as.matrix(df_mancova[, dv_vars]) ~ digital_class * sleep_composite.z + Age,
  data = df_mancova
)

cat("\n=== Multivariate MANCOVA (Digital × Sleep) ===\n")
print(summary(mancova_model, test = "Pillai"))

# Follow-up ANCOVAs
ancova_results <- map(dv_vars, function(dv) {
  model <- aov(as.formula(paste(dv, "~ digital_class * sleep_composite.z + Age")),
               data = df_mancova)
  tidy(model)
})

# Combine results & adjust p-values
ancova_combined <- bind_rows(ancova_results, .id = "DV") %>%
  filter(term != "Residuals") %>%
  mutate(p_adj = p.adjust(p.value, method = "holm"))

cat("\n=== ANCOVA Results with Holm correction (Digital × Sleep) ===\n")
print(ancova_combined)
...
```{r}
# -------------------------------------------------------------
# Test if sleep moderates the effect of digital immersion
# on executive functioning (TEXI) and impulsivity (Barretts)
# -------------------------------------------------------------
library(dplyr)
library(broom)
library(interactions)  # for visualization & simple slopes (optional)
...
# -------------------------------------------------------------
# Model 1: TEXI (Executive Functioning)
# -------------------------------------------------------------
model_texi <- lm(texi_sum_score.z ~ screen_behavior_pca_score * sleep_composite.z + Age,
                 data = mod_df)
summary(model_texi)

# -------------------------------------------------------------
# Model 1: Barrets (Executive Functioning)
# -------------------------------------------------------------
model_barrets <- lm(barrets.z ~ screen_behavior_pca_score * sleep_composite.z + Age,
                    data = mod_df)
summary(model_barrets)
```



```{r}
 # Count participants in the top quartile for both
high_overlap <- df1 %>%
     filter(digital_quartile == 4, sleep_quartile == 4)
 
 n_overlap <- nrow(high_overlap)
 total <- nrow(df1)
 percent_overlap <- round(100 * n_overlap / total, 1)
 
 cat("Top quartile overlap:", n_overlap, "out of", total, 
     "participants (", percent_overlap, "%)\n")
...
cat("=== VIF for TEXI Model ===\n")
vif_texi <- vif(model_texi)
print(vif_texi)

# Check if any VIF > 5 (moderate concern) or > 10 (serious concern)
cat("\nVIF Warning Flags:\n")
if(any(vif_texi > 10)) {
  cat("SERIOUS: VIF > 10 detected!\n")
  print(vif_texi[vif_texi > 10])
} else if(any(vif_texi > 5)) {
  cat("MODERATE: VIF > 5 detected\n")
  print(vif_texi[vif_texi > 5])
} else {
  cat("All VIF values < 5 (acceptable)\n")
}

# -------------------------------------------------------------
# Model 2: Barrets (Executive Functioning)
# -------------------------------------------------------------
model_barrets <- lm(barrets.z ~ screen_behavior_pca_score * sleep_composite.z + Age,
                    data = mod_df)

cat("\n\n=== VIF for Barrets Model ===\n")
vif_barrets <- vif(model_barrets)
print(vif_barrets)

# Check if any VIF > 5 (moderate concern) or > 10 (serious concern)
cat("\nVIF Warning Flags:\n")
if(any(vif_barrets > 10)) {
  cat("SERIOUS: VIF > 10 detected!\n")
  print(vif_barrets[vif_barrets > 10])
} else if(any(vif_barrets > 5)) {
  cat("MODERATE: VIF > 5 detected\n")
  print(vif_barrets[vif_barrets > 5])
} else {
  cat("All VIF values < 5 (acceptable)\n")
}

# -------------------------------------------------------------
# Compare VIF values side by side
# -------------------------------------------------------------
cat("\n\n=== VIF Comparison ===\n")
comparison <- data.frame(
  Predictor = names(vif_texi),
  TEXI_VIF = round(vif_texi, 2),
  Barrets_VIF = round(vif_barrets, 2)
)
print(comparison)
```
